...
public class AddReservation {
    ...
    private void showAddReservation() {
        ...
        ModernTextField resNo = new ModernTextField();  resNo.setPlaceholder("e.g., RES-001");
        ModernTextField name  = new ModernTextField();  name.setPlaceholder("Guest full name");
        ModernTextField addr  = new ModernTextField();  addr.setPlaceholder("Guest address");
        ModernTextField cont  = new ModernTextField();  cont.setPlaceholder("+94 XX XXX XXXX");
        ...
        clearBtn.addActionListener(e -> {
            resNo.setText(""); name.setText(""); addr.setText(""); cont.setText("");
        ...
        saveBtn.addActionListener(e -> {
            ...
            try {
                ...
                // ── Check duplicate in DB cache ──────────────────────────
                if (mainSystem.getReservations().containsKey(resNo.getText().trim())) {
                    JOptionPane.showMessageDialog(null, "Reservation number already exists!",
                            "Duplicate Entry", JOptionPane.WARNING_MESSAGE);
                    return;
                }

                // ── Build reservation object ─────────────────────────────
                OceanResortSystem.Reservation r = new OceanResortSystem.Reservation(
                    resNo.getText().trim(),
                    name.getText().trim(),
                    addr.getText().trim(),
                    cont.getText().trim(),
                ...
                boolean saved = mainSystem.insertReservation(r);

                if (saved) {
                    JOptionPane.showMessageDialog(null,
                            "✓ Reservation saved to database successfully!",
    ...
    private void addFormRow(JPanel container, GridBagConstraints gbc,
